...
object SlackRequestHandler {
    ...
    private suspend fun handleViewSubmission(
        ...
        val fields = payload.view.state.values["register-block"] ?: run {
            logger.warn("Missing register-block fields")
            return
        }
        val email = fields["email"]?.value ?: run {
            logger.warn("Missing email field")
            return
        }
        if (!email.matches(Regex("^$VALID_EMAIL_ADDRESS_REGEX$"))) {
            logger.warn("Invalid email address")
            return
        }
        val vaultAccountId = fields["vault-account-id"]?.value ?: run {
            logger.warn("Missing vault account id")
            return
        }
        val currency = fields["currency"]?.selectedOption?.value ?: "USD"
